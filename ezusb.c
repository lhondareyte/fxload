...
/*
 * Load a FX3 'img' file into target RAM. The backend is the open backend
 ...
 */
static int fx3_load_ram (struct ezusb_backend *backend, const char *path, int stage)
{
    int				image_fd;
 ...
 */
int ezusb_load_ram (struct ezusb_backend *backend, const char *path, const char *type, int stage)
{
    FILE			*image;
    ...
    if (strcmp(type, "fx3") == 0)
 	return fx3_load_ram (backend, path, stage);
